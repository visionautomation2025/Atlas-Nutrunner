...
class OptionMonitor:
    ...
    def store_option_data(self, data):
        ...
        try:
            with self.get_db_connection() as conn:
                ...
                current_expiry = data["records"]["expiryDates"][0]  # Get current expiry
                ...
                """
                prev_iv_data = pd.read_sql_query(prev_iv_query, conn)
            ...
            if count > 0:
                self.status_label.config(
                    text=f"Database Updated Successfully at {current_time}",
                    foreground="green"
                )
            else:
                self.status_label.config(
    ...
    def show_ltp_analysis(self):
        """Show LTP analysis graphs for highest volume CE and PE"""
        graph_window = tk.Toplevel(self.root)
        graph_window.title("LTP Analysis")
        ...
        try:
            with self.get_db_connection() as conn:
                ...
                ce_query = f"""
                SELECT date_time, ltp, MAX(ltp) OVER () as resistance,
                       MIN(ltp) OVER () as support
                FROM nifty_option_chain_data
                WHERE date_time LIKE '{current_date}%'
                AND strike_price = {ce_strike}
                AND option_type = 'CE'
                ORDER BY date_time
                """
                ce_data = pd.read_sql_query(ce_query, conn)
                ce_data['time'] = pd.to_datetime(ce_data['date_time']).dt.strftime('%H:%M')
                
                ax1.plot(ce_data['time'], ce_data['ltp'], 'b-', label=f'CE {ce_strike}')
                ax1.axhline(y=ce_data['resistance'].iloc[0], color='r', linestyle='--', label='Resistance')
                ax1.axhline(y=ce_data['support'].iloc[0], color='g', linestyle='--', label='Support')
                
                ax1.set_title(f'CE Strike {ce_strike} LTP', fontsize=10)
                ax1.set_xlabel('Time')
                ax1.set_ylabel('LTP')
                ax1.tick_params(axis='x', rotation=45)
                ax1.legend()
                
                # Add CE analysis text
                ce_analysis = self.analyze_ltp_movement(ce_data)
                ax1.text(0.05, 0.95, ce_analysis, transform=ax1.transAxes, 
                ...
                FROM nifty_option_chain_data
                WHERE date_time LIKE '{current_date}%'
                AND strike_price = {pe_strike}
                AND option_type = 'PE'
                ORDER BY date_time
                """
                pe_data = pd.read_sql_query(pe_query, conn)
                pe_data['time'] = pd.to_datetime(pe_data['date_time']).dt.strftime('%H:%M')
                
                ax2.plot(pe_data['time'], pe_data['ltp'], 'r-', label=f'PE {pe_strike}')
                ax2.axhline(y=pe_data['resistance'].iloc[0], color='r', linestyle='--', label='Resistance')
                ax2.axhline(y=pe_data['support'].iloc[0], color='g', linestyle='--', label='Support')
                
                ax2.set_title(f'PE Strike {pe_strike} LTP', fontsize=10)
                ax2.set_xlabel('Time')
                ax2.set_ylabel('LTP')
                ax2.tick_params(axis='x', rotation=45)
                ax2.legend()
                
                # Add PE analysis text
                pe_analysis = self.analyze_ltp_movement(pe_data)
                ax2.text(0.05, 0.95, pe_analysis, transform=ax2.transAxes, 
                        verticalalignment='top', bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
            ...
            messagebox.showerror("Error", f"Error creating LTP analysis: {str(e)}")

    def analyze_ltp_movement(self, data):
        """Analyze LTP movement and generate insights"""
        try:
            current_ltp = data['ltp'].iloc[-1]
            resistance = data['resistance'].iloc[0]
            support = data['support'].iloc[0]
            
            # Calculate recent trend
            recent_data = data['ltp'].tail(5)
            trend = "upward" if recent_data.is_monotonic_increasing else \
                   "downward" if recent_data.is_monotonic_decreasing else "sideways"
            
            # Generate analysis
            analysis = "LTP Analysis:\n"
            
            # Check proximity to support/resistance
            resistance_proximity = (resistance - current_ltp) / resistance * 100
            support_proximity = (current_ltp - support) / support * 100
            
            if resistance_proximity < 2:  # Within 2% of resistance
                analysis += "Near Resistance\n"
                if trend == "upward":
                    analysis += "Potential Breakout\n"
                else:
                    analysis += "Resistance Holding\n"
            elif support_proximity < 2:  # Within 2% of support
                analysis += "Near Support\n"
                if trend == "downward":
                    analysis += "Support May Break\n"
                else:
                    analysis += "Support Holding\n"
            else:
                analysis += "In Consolidation\n"
