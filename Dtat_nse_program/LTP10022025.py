...
class NiftyApp:
    ...
    def setup_ui(self):
        ...
        # Style for tab and notebook background
        style = ttk.Style()

        # Customize Tab background
        style.configure('TNotebook', background='#2C3E50')  # Background color of the whole notebook
        style.configure('TNotebook.Tab', background='#1F3A58', foreground='red', padding=[10, 5])
        style.map('TNotebook.Tab', background=[('selected', '#006D70')], foreground=[('selected', 'red')])
...
# Configure columns for PE Treeview
    ...
    # Clear the second tab
        ...
        graph1_frame = ttk.Frame(self.tab2)
        graph1_frame.grid(row=0, column=1, sticky="nsew")
        
        # Create a frame for the entry widgets
        entry_frame = ttk.Frame(self.tab2)
        entry_frame.grid(row=1, column=0, columnspan=2, pady=10)
            
         # Create Entry widgets to display correlation coefficients
        self.entry_corr_ce = ttk.Entry(entry_frame)
        self.entry_corr_pe = ttk.Entry(entry_frame)
        self.entry_corr_ce21 = ttk.Entry(entry_frame)
        self.entry_corr_pe21 = ttk.Entry(entry_frame)

        # Position the Entry widgets
        self.entry_corr_ce.grid(row=0, column=0, padx=15, pady=15)
        self.entry_corr_pe.grid(row=0, column=1, padx=15, pady=15)
        self.entry_corr_ce21.grid(row=1, column=0, padx=15, pady=15)
        self.entry_corr_pe21.grid(row=1, column=1, padx=15, pady=15)
        
        self.entry_prediction = ttk.Entry(entry_frame)
        self.entry_prediction.grid(row=0, column=2, padx=15, pady=15, rowspan=2)
        ...
        # Query to fetch data for CE and PE
        query_ce = f"""
        ...
        """
        askbidquery_ce2 = f"""
        SELECT date_time,ask_price,bid_price
        FROM option_ce_askbid
        WHERE strike_price = {strike_price_ce} AND date_time BETWEEN '{current_date} 09:00:00' AND '{current_date} 15:30:00'
        """
        askbidquery_pe2 = f"""
        SELECT date_time,ask_price,bid_price
        FROM option_pe_askbid
        WHERE strike_price = {strike_price_pe} AND date_time BETWEEN '{current_date} 09:00:00' AND '{current_date} 15:30:00'
        """
        ...
        with sqlite3.connect('E:/nifty_data.db') as conn:
            ...
            askbiddf_ce = pd.read_sql_query(askbidquery_ce2, conn)
            askbiddf_pe = pd.read_sql_query(askbidquery_pe2, conn)
        # Convert date_time to datetime format for all dataframes
        ...
        askbiddf_ce['date_time'] = pd.to_datetime(askbiddf_ce['date_time'])
        askbiddf_pe['date_time'] = pd.to_datetime(askbiddf_pe['date_time'])
        # -------- First CE Graph --------
        frame_ce_graph = ttk.Frame(graph_frame)
        frame_ce_graph.grid(row=0, column=0, padx=10, pady=10)
              

        fig_ce, ax_ce = plt.subplots(figsize=(5, 3))
        fig_ce.patch.set_facecolor('white')
        fig_ce.patch.set_facecolor('lightblue')
        ...
        frame_pe_graph.grid(row=0, column=1, padx=10, pady=10)

        fig_pe, ax_pe = plt.subplots(figsize=(5, 3))
        fig_pe.patch.set_facecolor('lightblue')
        ax_pe.plot(df_pe['date_time'], df_pe['changein_oi'], label="PE Change in OI", color='green')
        ax_pe.set_xlabel("Time")
        ax_pe.set_ylabel("Change in OI", color='green')
        ax_pe.tick_params(axis='y', labelcolor='green')
        ax_pe.set_facecolor('black')
        ax_pe_ltp = ax_pe.twinx()
        ax_pe_ltp.plot(df_pe['date_time'], df_pe['ltp'], label="PE LTP", color='red')
        ...
        frame_ce_graph2.grid(row=1, column=0, padx=10, pady=10)

        fig_ce2, ax_ce2 = plt.subplots(figsize=(5, 3))
        fig_ce2.patch.set_facecolor('lightblue')
        ...
        frame_pe_graph2.grid(row=1, column=1, padx=10, pady=10)

        fig_pe2, ax_pe2 = plt.subplots(figsize=(5, 3))
        fig_pe2.patch.set_facecolor('lightblue')
        ax_pe2.plot(df_pe2['date_time'], df_pe2['changein_oi'], label="PE Change in OI", color='green')
        ax_pe2.set_xlabel("Time")
        ax_pe2.set_ylabel("Change in OI", color='green')
        ax_pe2.tick_params(axis='y', labelcolor='green')
        ax_pe2.tick_params(axis='y', labelcolor='green')
        ax_pe2.set_facecolor('black')
        ax_pe2_ltp = ax_pe2.twinx()
        ax_pe2_ltp.plot(df_pe2['date_time'], df_pe2['ltp'], label="PE LTP", color='red')
        ax_pe2_ltp.set_ylabel("LTP", color='red')
        ax_pe2_ltp.tick_params(axis='y', labelcolor='red')
        ...
        SELECT date_time, iv, ltp
        FROM option_pe_data
        WHERE strike_price = {strike_price_pe21} AND date_time BETWEEN '{current_date} 09:00:00' AND '{current_date} 20:30:00'
        """
#             messagebox.showinfo("Title", f"This second: {strike_price_pe2ltp}")
        # Fetch data from the database
        with sqlite3.connect('E:/nifty_data.db') as conn:
            df_celtp = pd.read_sql_query(query_celtp, conn)
            df_peltp = pd.read_sql_query(query_peltp, conn)
            df_ce2ltp = pd.read_sql_query(query_ce2ltp, conn)
            df_pe2ltp = pd.read_sql_query(query_pe2ltp, conn)
        
        # Compute correlation coefficients
        corr_celtp = df_celtp[['ltp', 'iv']].corr().iloc[0, 1]
        corr_peltp = df_peltp[['ltp', 'iv']].corr().iloc[0, 1]
        corr_ce21ltp = df_ce2ltp[['ltp', 'iv']].corr().iloc[0, 1]
        corr_pe21ltp = df_pe2ltp[['ltp', 'iv']].corr().iloc[0, 1]
#         self.entry_corr_ce.place(x=23, y=692)
# 
#         # Place the second entry at the same position, or adjust the y-coordinate slightly to avoid overlap
#         self.entry_corr_ce21.place(x=23, y=692 + 30)
#         self.entry_corr_ce21.place(x=23, y=692 + 60)  # Adjust for vertical spacing
#         self.entry_corr_pe21.place(x=23, y=692 + 90)  # Adjust for vertical spacing
#         
        # Insert correlation values into Entry widgets
        self.entry_corr_ce.insert(0, f"{corr_celtp:.4f}")
        self.entry_corr_pe.insert(0, f"{corr_peltp:.4f}")
        self.entry_corr_ce21.insert(0, f"{corr_ce21ltp:.4f}")
        self.entry_corr_pe21.insert(0, f"{corr_pe21ltp:.4f}")
        
        
        prediction = self.make_prediction(corr_celtp, corr_peltp, corr_ce21ltp, corr_pe21ltp)
        self.entry_prediction.insert(0, prediction)

    def make_prediction(self, corr_celtp, corr_peltp, corr_ce21ltp, corr_pe21ltp):
        # Define prediction logic
        if corr_celtp > 0.7 and corr_peltp > 0.7:
            return "Strong Bullish Trend"
        elif corr_celtp < -0.7 and corr_peltp < -0.7:
            return "Strong Bearish Trend"
        elif abs(corr_celtp) < 0.3 and abs(corr_peltp) < 0.3:
...
#         #first ask/bid ce
#         
#         frame_ce_graph3 = ttk.Frame(graph1_frame)
#         frame_ce_graph3.grid(row=0, column=0, padx=10, pady=10)
#               
# 
#         fig_ce3, ax_ce3 = plt.subplots(figsize=(3, 3))
#         fig_ce3.patch.set_facecolor('white')
#         fig_ce3.patch.set_facecolor('orange')
#         
#         ax_ce3.plot(askbiddf_ce['date_time'], askbiddf_ce['ask_price'], label="ask_price", color='green')
#         ax_ce3.set_xlabel("Time")
#         ax_ce3.set_ylabel("Change in OI", color='green')
#         ax_ce3.tick_params(axis='y', labelcolor='green')
#         ax_ce3.set_facecolor('black')
# 
#         ax_ce_ltp3 = ax_ce3.twinx()
#         ax_ce_ltp3.plot(askbiddf_ce['date_time'], askbiddf_ce['bid_price'], label="bid_price", color='red')
#         ax_ce_ltp3.set_ylabel("LTP", color='red')
#         ax_ce_ltp3.tick_params(axis='y', labelcolor='red')
# 
#         lines_ce3, labels_ce3 = ax_ce3.get_legend_handles_labels()
#         lines_ltp_ce3, labels_ltp_ce3 = ax_ce_ltp3.get_legend_handles_labels()
#         ax_ce3.legend(lines_ce3+ lines_ltp_ce3, labels_ce3 + labels_ltp_ce3, loc="upper left")
# 
#         ax_ce3.set_title(f"Ask/bid CE Graph (Strike Price: {strike_price_ce})")
# 
#         canvas_ce3 = FigureCanvasTkAgg(fig_ce3, master=frame_ce_graph3)
#         canvas_ce3_widget = canvas_ce3.get_tk_widget()
#         canvas_ce3_widget.pack()
#         
#         # ask bid pe graph
#         frame_pe_graph3 = ttk.Frame(graph1_frame)
#         frame_pe_graph3.grid(row=1, column=0, padx=10, pady=10)
# 
#         fig_pe3, ax_pe3 = plt.subplots(figsize=(3, 3))
#         fig_pe3.patch.set_facecolor('orange')
#         ax_pe3.plot(askbiddf_pe['date_time'], askbiddf_pe['ask_price'], label="askprice", color='green')
#         ax_pe3.set_xlabel("Time")
#         ax_pe3.set_ylabel("Change in OI", color='green')
#         ax_pe3.tick_params(axis='y', labelcolor='green')
#         ax_pe3.tick_params(axis='y', labelcolor='green')
#         ax_pe3.set_facecolor('black')
#         ax_pe3_ltp = ax_pe3.twinx()
#         ax_pe3_ltp.plot(askbiddf_pe['date_time'], askbiddf_pe['bid_price'], label="bid price", color='red')
#         ax_pe3_ltp.set_ylabel("LTP", color='red')
#         ax_pe3_ltp.tick_params(axis='y', labelcolor='red')
# 
#         lines_pe3, labels_pe3 = ax_pe3.get_legend_handles_labels()
#         lines_ltp_pe3, labels_ltp_pe3 = ax_pe3_ltp.get_legend_handles_labels()
#         ax_pe3.legend(lines_pe3 + lines_ltp_pe3, labels_pe3 + labels_ltp_pe3, loc="upper left")
# 
#         ax_pe3.set_title(f"Ask/bid PE Graph (Strike Price: {strike_price_pe})")
# 
#         canvas_pe3 = FigureCanvasTkAgg(fig_pe3, master=frame_pe_graph3)
#         canvas_pe3_widget = canvas_pe3.get_tk_widget()
#         canvas_pe3_widget.pack()
#         
     

    def setup_third_tab(self):
    # Create a grid layout for the third tab
        self.tab3.columnconfigure(0, weight=1)
        self.tab3.columnconfigure(1, weight=1)
        self.tab3.rowconfigure(0, weight=1)

        # CE Frame
        ce_frame = ttk.LabelFrame(self.tab3, text="Call Options Ask/Bid (CE)", padding=5)
        ce_frame.grid(row=0, column=0, sticky="nsew", padx=(5, 2), pady=5)

        # PE Frame
        pe_frame = ttk.LabelFrame(self.tab3, text="Put Options Ask/Bid (PE)", padding=5)
        pe_frame.grid(row=0, column=1, sticky="nsew", padx=(2, 5), pady=5)

        # Create treeviews with columns for ask/bid data
        columns = ("Strike", "Bid Qty", "Bid Price", "Ask Price", "Ask Qty", "LTP")

        # CE Treeview
        self.ce_askbid_tree = CustomTreeview(ce_frame, columns=columns, show="headings", height=15)
        ce_scroll = ttk.Scrollbar(ce_frame, orient=tk.VERTICAL, command=self.ce_askbid_tree.yview)
        self.ce_askbid_tree.configure(yscrollcommand=ce_scroll.set)

        # PE Treeview
        self.pe_askbid_tree = CustomTreeview(pe_frame, columns=columns, show="headings", height=15)
        pe_scroll = ttk.Scrollbar(pe_frame, orient=tk.VERTICAL, command=self.pe_askbid_tree.yview)
        self.pe_askbid_tree.configure(yscrollcommand=pe_scroll.set)

        # Configure columns for CE Treeview
        for col in columns:
            self.ce_askbid_tree.heading(col, text=col)
            self.ce_askbid_tree.column(col, width=100, anchor="center")

        # Configure columns for PE Treeview
        for col in columns:
            self.pe_askbid_tree.heading(col, text=col)
            self.pe_askbid_tree.column(col, width=100, anchor="center")

        # Pack treeviews and scrollbars
        self.ce_askbid_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        ce_scroll.pack(side=tk.RIGHT, fill=tk.Y)
        self.pe_askbid_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        pe_scroll.pack(side=tk.RIGHT, fill=tk.Y)
    ...
    def get_relevant_strikes(self, atm_strike):
        """
        Get relevant strike prices around ATM strike
        Returns strikes within ±450 points of ATM (9 strikes above and below)
        """
        strike_step = 50  # Standard step size for NIFTY options
        strikes = []
        
        # Add 9 strikes below ATM
        for i in range(9):
            strikes.append(atm_strike - (i * strike_step))
        
        # Add 9 strikes above ATM
        for i in range(1, 10):
            strikes.append(atm_strike + (i * strike_step))
        
        return sorted(strikes)  # Return sorted list of strikes
...
#             messagebox.showerror("Error", f"Failed to fetch data: {str(e)}")
    ...
    def insert_ce_askbid_data(self, date_time, strike_price, bid_qty, bid_price, ask_price, ask_qty, ltp):
        self.cursor.execute('''
            INSERT INTO option_ce_askbid (date_time, strike_price, bid_qty, bid_price, ask_price, ask_qty, ltp)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (date_time, strike_price, bid_qty, bid_price, ask_price, ask_qty, ltp))
        self.conn.commit()

    def insert_pe_askbid_data(self, date_time, strike_price, bid_qty, bid_price, ask_price, ask_qty, ltp):
        self.cursor.execute('''
            INSERT INTO option_pe_askbid (date_time, strike_price, bid_qty, bid_price, ask_price, ask_qty, ltp)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ''', (date_time, strike_price, bid_qty, bid_price, ask_price, ask_qty, ltp))
        self.conn.commit()
    ...
    def process_and_display_data(self, data):
        try:
            ...
            for tree in (self.ce_tree, self.pe_tree, self.ce_askbid_tree, self.pe_askbid_tree):
            ...
            # Initialize strike_data dictionary
            strike_data = {item["strikePrice"]: item for item in data["records"]["data"] if item["expiryDate"] == current_expiry}
            ...
            for strike in relevant_strikes:
                ...
                if "CE" in item:
                    ...
                    ce_tree_item = self.ce_tree.insert("", "end", values=ce_values, tags=["atm" if strike == atm_strike else "above_atm" if strike > atm_strike else "below_atm"])

                    ce_askbid_values = (
                        strike,
                        ce_data.get("bidQty", 0),
                        ce_data.get("bidprice", 0),  # Corrected key
                        ce_data.get("askPrice", 0),
                        ce_data.get("askQty", 0),
                        ce_data.get("lastPrice", 0)
                    )
                    ce_askbid_item = self.ce_askbid_tree.insert("", "end", values=ce_askbid_values, tags=["atm" if strike == atm_strike else "above_atm" if strike > atm_strike else "below_atm"])
                    ...
                    ce_data_list.append((strike, ce_data.get("totalTradedVolume", 0), 
                                       ce_data.get("openInterest", 0), 
                                       ce_data.get("changeinOpenInterest", 0), 
                                       ce_data.get("lastPrice", 0), 
                                       ce_change_value))
                ...
                if "PE" in item:
                    ...
                    pe_tree_item = self.pe_tree.insert("", "end", values=pe_values, tags=["atm" if strike == atm_strike else "above_atm" if strike > atm_strike else "below_atm"])

                    pe_askbid_values = (
                        strike,
                        pe_data.get("bidQty", 0),
                        pe_data.get("bidprice", 0),  # Corrected key
                        pe_data.get("askPrice", 0),
                        pe_data.get("askQty", 0),
                        pe_data.get("lastPrice", 0)
                    )
                    pe_askbid_item = self.pe_askbid_tree.insert("", "end", values=pe_askbid_values, tags=["atm" if strike == atm_strike else "above_atm" if strike > atm_strike else "below_atm"])
                    ...
                    pe_data_list.append((strike, pe_data.get("totalTradedVolume", 0), 
                                       pe_data.get("openInterest", 0), 
                                       pe_data.get("changeinOpenInterest", 0), 
                                       pe_data.get("lastPrice", 0), 
                                       pe_change_value))
...
#             self.update_volume_tracking(ce_data_list, pe_data_list)
