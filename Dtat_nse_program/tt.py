...
#         ce_frame = ttk.LabelFrame(chain_frame, text="Call Options (CE)", padding=0)
        ...
        # CE Treeview
        self.ce_tree = CustomTreeview(ce_frame, columns=columns, show="headings", height=19)  # Height set to 10
...
#         
#         self.ce_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=0, pady=0)  # Adjust padx and pady as needed
#         ce_scroll.pack(side=tk.RIGHT, fill=tk.Y, padx=0, pady=0)  # Adjust padx and pady as needed
        ...
        # PE Treeview
        self.pe_tree = CustomTreeview(pe_frame, columns=columns, show="headings", height=19)  # Height set to 10
...
#         self.pe_tree.configure(yscrollcommand=pe_scroll.set)
        ...
        volume_ce_frame.place(x=500,y=100)
        ...
        volume_pe_frame.place(x=500,y=236) 
...
#         pe_scroll.pack(side=tk.RIGHT, fill=tk.Y)
    ...
    # Extract top 2 high volume CE strike prices

    top_2_ce_data = top_ce_data[:2]  # Get the top 2


    # Prepare data for plotting

    top_2_strikes = [item[0] for item in top_2_ce_data]

    top_2_ltp = [item[4] for item in top_2_ce_data]  # LTP

    top_2_change_in_oi = [item[3] for item in top_2_ce_data]  # Chng in OI


    # Call the plotting function

    self.plot_top_2_ce_data(top_2_strikes, top_2_ltp, top_2_change_in_oi)        
    ...
    def plot_top_2_ce_data(self, strikes, ltps, change_in_oi):
    # Create a new window for plotting
    plot_window = tk.Toplevel(self.root)
    plot_window.title("Top 2 High Volume CE Strike Prices")
    ...
    # Plot LTP
    ax1.bar(strikes, ltps, color='blue')
    ax1.set_title("Last Traded Price (LTP) for Top 2 CE Strikes")
    ax1.set_xlabel("Strike Prices")
    ax1.set_ylabel("LTP")
    ax1.grid(True)

    # Plot Change in OI
    ax2.bar(strikes, change_in_oi, color='orange')
    ax2.set_title("Change in Open Interest (Chng in OI) for Top 2 CE Strikes")
