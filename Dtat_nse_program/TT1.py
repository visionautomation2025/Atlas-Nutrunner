...
class NiftyApp:
        ...
        # Create notebook (tab container)
            ...
            self.setup_volume_tab_contents()
            
            def setup_volume_tab_contents(self):
            # Create a frame for combo boxes with padding
                combo_frame = ttk.Frame(self.volume_tab, padding="20 20 20 20")
          ...
          def fetch_top_volume_strikes(self):
            try:
                # Get today's date in YYYY-MM-DD format
                today = datetime.now().strftime('%Y-%m-%d')
                
                # Fetch top 5 CE strike prices by volume for today
                self.cursor.execute("""
                   SELECT strike_price, MAX(volume) as max_volume
                FROM option_ce_data
                ...
                LIMIT 5
                 """, (f"{today}%",))
                ce_strikes = [str(row[0]) for row in self.cursor.fetchall()]
                
                # Fetch top 5 PE strike prices by volume for today
                self.cursor.execute("""
                    SELECT strike_price, MAX(volume) as max_volume
                FROM option_pe_data
                WHERE date_time LIKE ?
                ...
                LIMIT 5
                """, (f"{today}%",))
                pe_strikes = [str(row[0]) for row in self.cursor.fetchall()]

                # Update combo box values
                self.ce_combo['values'] = ce_strikes
                self.pe_combo['values'] = pe_strikes

                # Set default selections if available
                if ce_strikes:
                    self.ce_combo.set(ce_strikes[0])
                if pe_strikes:
                    self.pe_combo.set(pe_strikes[0])

            except sqlite3.Error as e:
                self.status_label.config(text=f"Database error: {str(e)}")
                messagebox.showerror("Database Error", f"Failed to fetch strike prices: {str(e)}")

        def setup_combo_boxes(self):
        # Create a frame for combo boxes
            combo_frame = ttk.Frame(self.root)
            combo_frame.pack(fill=tk.X, padx=10, pady=5)

            # CE Combo Box
            ttk.Label(combo_frame, text="Top Volume CE Strikes:").pack(side=tk.LEFT, padx=(0, 5))
            self.ce_combo = ttk.Combobox(combo_frame, width=10, state="readonly")
            self.ce_combo.pack(side=tk.LEFT, padx=(0, 20))

            # PE Combo Box
            ttk.Label(combo_frame, text="Top Volume PE Strikes:").pack(side=tk.LEFT, padx=(0, 5))
            self.pe_combo = ttk.Combobox(combo_frame, width=10, state="readonly")
            self.pe_combo.pack(side=tk.LEFT)
        ...
        def __init__(self, root):
            ...
            self.setup_database()
            self.setup_tabs()                      # Initialize the database first
            self.setup_ui()
            
    #         self.setup_combo_boxes()# Then set up the UI
        ...
        def setup_ui(self):
            ...
            main_container = ttk.Frame(self.main_tab)  # Changed from self.root
            main_container.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
            # Header frame
            ...
            # CE Treeview
            self.ce_tree = CustomTreeview(ce_frame, columns=columns, show="headings", height=15)  # Height set to 10
            ...
            # PE Treeview
            self.pe_tree = CustomTreeview(pe_frame, columns=columns, show="headings", height=15)  # Height set to 10
            ...
            for tree in (self.ce_tree, self.pe_tree):
                for col in columns:
                    ...
                    tree.column(col, width=80, anchor="center")  # Width adjusted to 80 for compact layout

            # Pack treeviews and scrollbars
            self.ce_tree.pack(side=tk.LEFT, fill=tk.NONE, expand=False )
            ce_scroll.pack(side=tk.RIGHT, fill=tk.Y)
            self.pe_tree.pack(side=tk.LEFT, fill=tk.NONE, expand=False)
            ...
            # IV Track button
            self.iv_button = ttk.Button(main_container, text="IV Track", command=self.open_iv_track_window)
            self.iv_button.pack(side=tk.BOTTOM, pady=10)
