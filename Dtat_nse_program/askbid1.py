...
class NiftyApp:
    def __init__(self, root):
        ...
        self.root.geometry("2100x1000")  # Set geometry to 1000x1000
...
#       raph1_frame.pack(fill=tk.BOTH, expand=True)
        ...
        with sqlite3.connect('E:/nifty_data.db') as conn:
            ...
            askbiddf_ce = pd.read_sql_query(askbidquery_ce, conn)
            askbiddf_pe = pd.read_sql_query(askbidquery_pe, conn)
        ...
        askbiddfdf_ce['date_time'] = pd.to_datetime(askbiddfdf_ce['date_time'])
        askbiddfdf_pe['date_time'] = pd.to_datetime(askbiddfdf_pe['date_time'])
        ...
        canvas_ce3_widget.pack()
                ...
                # -------- First Ask/bid PE Graph --------
        
        frame_pe_graph3 = ttk.Frame(graph1_frame)
        frame_pe_graph3.grid(row=1, column=0, padx=10, pady=10)

        fig_pe3, ax_pe3 = plt.subplots(figsize=(5, 3))
        fig_pe3.patch.set_facecolor('lightblue')
        ax_pe3.plot(df_pe2['date_time'], df_pe2['changein_oi'], label="PE Change in OI", color='green')
        ...
        ax_pe3_ltp.plot(df_pe2['date_time'], df_pe2['ltp'], label="PE LTP", color='red')
        ...
        ax_pe3.set_title(f"Ask/bid PE Graph (Strike Price: {strike_price_pe2})")
